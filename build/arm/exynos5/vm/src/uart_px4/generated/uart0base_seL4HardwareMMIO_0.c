...
int uart0base_wrap_ptr(dataport_ptr_t *p, void *ptr) {
    ...
    off_t _camkes_offset_608 = (off_t)((uintptr_t)ptr - (uintptr_t)uart0base);
    if (_camkes_offset_608 < sizeof(Buf)) {
        p->id = 21;
        p->offset = _camkes_offset_608;
        return 0;
    } else {
        return -1;
    }
