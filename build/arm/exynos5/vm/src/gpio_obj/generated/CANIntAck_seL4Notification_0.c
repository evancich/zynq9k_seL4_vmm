 ...
 */
static camkes_error_handler_t CANIntAck_error_handler_fn UNUSED;

camkes_error_handler_t CANIntAck_register_error_handler(
    camkes_error_handler_t handler) {
    camkes_error_handler_t old = CANIntAck_error_handler_fn;
    CANIntAck_error_handler_fn = handler;
    return old;
}

static camkes_error_action_t UNUSED CANIntAck_error_handler(camkes_error_t *error) {
    if (CANIntAck_error_handler_fn == NULL) {
        /* No registered handler; invoke the generic error handler. */
        return camkes_error(error);
    }
    return CANIntAck_error_handler_fn(error);
}
...
static int lock(void) {
    int result = sync_bin_sem_bare_wait(34, &lock_count);
    __sync_synchronize();
    return result;
}
...
static int unlock(void) {
    __sync_synchronize();
    return sync_bin_sem_bare_post(34, &lock_count);
}

static void (*callback)(void*);
static void *callback_arg;

int CANIntAck__run(void) {
    while (true) {
        seL4_Wait(32, NULL);
        ...
        if (cb == NULL) {
            ...
            if (handoff_value != INT_MAX) {
                sync_sem_bare_post(33, &handoff_value);
            }
        }
...
static int poll(void) {
    return sync_sem_bare_trywait(33, &handoff_value) == 0;
}

int CANIntAck_poll(void) {
    return poll();
}
...
#endif
    if (unlikely(sync_sem_bare_wait(33, &handoff_value) != 0)) {
        ERR(CANIntAck_error_handler, ((camkes_error_t){
                .type = CE_OVERFLOW,
                .instance = "gpio_obj",
                .interface = "CANIntAck",
...
int CANIntAck_reg_callback(void (*cb)(void*), void *arg) {
