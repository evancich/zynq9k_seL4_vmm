 ...
 */
static camkes_error_handler_t Int_error_handler_fn UNUSED;

camkes_error_handler_t Int_register_error_handler(
    camkes_error_handler_t handler) {
    camkes_error_handler_t old = Int_error_handler_fn;
    Int_error_handler_fn = handler;
    return old;
}

static camkes_error_action_t UNUSED Int_error_handler(camkes_error_t *error) {
    if (Int_error_handler_fn == NULL) {
        /* No registered handler; invoke the generic error handler. */
        return camkes_error(error);
    }
    return Int_error_handler_fn(error);
}
...
static int lock(void) {
    int result = sync_bin_sem_bare_wait(14, &lock_count);
    __sync_synchronize();
    return result;
}
...
static int unlock(void) {
    __sync_synchronize();
    return sync_bin_sem_bare_post(14, &lock_count);
}
    ...
    while (true) {
        ...
        if (cb == NULL) {
            ...
            if (handoff_value != INT_MAX) {
                sync_sem_bare_post(13, &handoff_value);
            }
        }
...
static int poll(void) {
    return sync_sem_bare_trywait(13, &handoff_value) == 0;
}
...
#endif
    if (unlikely(sync_sem_bare_wait(13, &handoff_value) != 0)) {
        ERR(Int_error_handler, ((camkes_error_t){
                .type = CE_OVERFLOW,
                .instance = "can_obj",
                .interface = "Int",
...
int Int_reg_callback(void (*cb)(void*), void *arg) {
