 ...
 */
static camkes_error_handler_t restart_event_error_handler_fn UNUSED;

camkes_error_handler_t restart_event_register_error_handler(
    camkes_error_handler_t handler) {
    camkes_error_handler_t old = restart_event_error_handler_fn;
    restart_event_error_handler_fn = handler;
    return old;
}

static camkes_error_action_t UNUSED restart_event_error_handler(camkes_error_t *error) {
    if (restart_event_error_handler_fn == NULL) {
        /* No registered handler; invoke the generic error handler. */
        return camkes_error(error);
    }
    return restart_event_error_handler_fn(error);
}
...
static int lock(void) {
    int result = sync_bin_sem_bare_wait(13, &lock_count);
    __sync_synchronize();
    return result;
}
...
static int unlock(void) {
    __sync_synchronize();
    return sync_bin_sem_bare_post(13, &lock_count);
}

static void (*callback)(void*);
static void *callback_arg;

int restart_event__run(void) {
    while (true) {
        seL4_Wait(11, NULL);
        ...
        if (cb == NULL) {
            ...
            if (handoff_value != INT_MAX) {
                sync_sem_bare_post(12, &handoff_value);
            }
        }
...
static int poll(void) {
    return sync_sem_bare_trywait(12, &handoff_value) == 0;
}

int restart_event_poll(void) {
    return poll();
}
...
#endif
    if (unlikely(sync_sem_bare_wait(12, &handoff_value) != 0)) {
        ERR(restart_event_error_handler, ((camkes_error_t){
                .type = CE_OVERFLOW,
                .instance = "vm",
                .interface = "restart_event",
...
int restart_event_reg_callback(void (*cb)(void*), void *arg) {
