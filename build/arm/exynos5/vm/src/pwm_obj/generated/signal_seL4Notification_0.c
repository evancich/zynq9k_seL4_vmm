 ...
 */
static camkes_error_handler_t signal_error_handler_fn UNUSED;

camkes_error_handler_t signal_register_error_handler(
    camkes_error_handler_t handler) {
    camkes_error_handler_t old = signal_error_handler_fn;
    signal_error_handler_fn = handler;
    return old;
}

static camkes_error_action_t UNUSED signal_error_handler(camkes_error_t *error) {
    if (signal_error_handler_fn == NULL) {
        /* No registered handler; invoke the generic error handler. */
        return camkes_error(error);
    }
    return signal_error_handler_fn(error);
}
...
static int lock(void) {
    int result = sync_bin_sem_bare_wait(23, &lock_count);
    __sync_synchronize();
    return result;
}
...
static int unlock(void) {
    __sync_synchronize();
    return sync_bin_sem_bare_post(23, &lock_count);
}

static void (*callback)(void*);
static void *callback_arg;

int signal__run(void) {
    while (true) {
        seL4_Wait(21, NULL);
        ...
        if (cb == NULL) {
            ...
            if (handoff_value != INT_MAX) {
                sync_sem_bare_post(22, &handoff_value);
            }
        }
...
static int poll(void) {
    return sync_sem_bare_trywait(22, &handoff_value) == 0;
}
...
#endif
    if (unlikely(sync_sem_bare_wait(22, &handoff_value) != 0)) {
        ERR(signal_error_handler, ((camkes_error_t){
                .type = CE_OVERFLOW,
                .instance = "pwm_obj",
                .interface = "signal",
...
int signal_reg_callback(void (*cb)(void*), void *arg) {
